...
import type { Expression, ExpressionNode as ExprNode } from '../types.js';
...
export const ExpressionNode: Component<ExpressionNodeProps> = (props) => {
  ...
  const shouldHighlight = createMemo(() =>
    isVariable() &&
    props.highlightedVars().has(props.expr as string)
  );
  ...
  const renderExpression = () => {
    ...
    if (isEditing()) {
      return (
        <input
          ...
          onBlur={saveEdit}
          class="esm-expression-edit"
          autoFocus
          aria-label={`Edit ${typeof props.expr === 'number' ? 'number' : 'variable'}`}
        />
      );
    }
    ...
    if (typeof props.expr === 'string') {
      return (
        <span class="esm-variable" title={`Variable: ${props.expr}`}>
          {props.expr}
        </span>
      );
    }
  ...
  const renderOperatorNode = (node: ExprNode) => {
    const { op, args } = node;
  ...
  const renderInfixOperator = (op: string, args: Expression[]) => (
    <span class="esm-infix-op">
      <For each={args}>
        {(arg, index) => (
          <>
            <Show when={index() > 0}>
  ...
  const renderMultiplication = (args: Expression[]) => (
    <span class="esm-multiplication">
      <For each={args}>
        {(arg, index) => (
          <>
            <Show when={index() > 0}>
  ...
  const renderFraction = (args: Expression[]) => (
    <span class="esm-fraction">
  ...
  const renderExponentiation = (args: Expression[]) => (
    <span class="esm-exponentiation">
      <span class="esm-base">
        <ExpressionNode
          expr={args[0]}
          path={[...props.path, 'args', 0]}
  ...
  const renderDerivative = (node: ExprNode) => (
    <span class="esm-derivative">
  ...
  const renderFunction = (name: string, args: Expression[]) => (
    <span class="esm-function">
  ...
  const renderComparison = (op: string, args: Expression[]) => (
    <span class="esm-comparison">
      <For each={args}>
        {(arg, index) => (
          <>
            <Show when={index() > 0}>
  ...
  const renderGenericFunction = (op: string, args: Expression[]) => (
    <span class="esm-generic-function">
